...
function InverseProtectedRoute({ outlet }: { outlet: React.ReactNode }) {
  ...
  if (user.isAuth) {
    return <Navigate to="/dashboard" replace state={{ from: location, unathorized: true }} />;
  }

  if (getUserError || getUserProfileError) {
